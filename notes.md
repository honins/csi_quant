...
- 本次为小型实验，仅做流程验证与报告生成
- 保持市场尊重：不修改核心逻辑与阈值
- 动态阈值相关A/B选项已移除，统一使用固定 final_threshold。

## 2025-10-19 半年分箱均值曲线与不确定性发现

### 回测与数据
- 回测区间: 2025-04-17 至 2025-10-17（以3月明细最大日期为终点回推6个月）
- 明细CSV: `results/csv/daily_details_rolling_backtest_20251019_193312.csv`
- 分箱方法: 分位数分箱，`bins=10`
- 可视化PNG: `results/plots/confidence_bins_mean_curve_rolling_backtest_6m_20251019_193312.png`
- 分箱摘要CSV: `results/csv/confidence_bins_summary_rolling_backtest_6m_20251019_193312.csv`

### 主要发现
- 中间区间近似线性：置信度约 `0.09–0.54` 内，分箱均值涨幅随置信度上升整体平滑递增；相较三个月期，半年期斜率略放缓（行情阶段混合更复杂）。
- 高区间饱和/回落：`0.71+` 段不再单调上升，出现先升后回落，说明高置信度样本混合了不同市场阶段（强趋势后的次日并非相对低点），均值受阶段分布影响更显著。
- IQR 解读：同一置信度分箱的 IQR 垂直跨度越大，表示该置信度对应的“未来最大涨幅”结果更分散、异质性更强，可视为更高的结果不确定性；IQR 描述的是结果分散度而非均值估计不确定性。
- 与均值估计不确定性区分：建议叠加 `mean ± 95%CI`（基于标准误 SEM），并标注每箱样本数 `count`，避免将结果分散度（IQR）与均值估计不确定性混为一谈。

### 对比补充
- 三个月期与半年期均存在显著正相关；半年期因市场阶段更混合，线性段的斜率与单调性略弱，但总体关系仍稳健。
